...
<%@page import="com.it.j2ee.modules.permission.entity.Menu"%>
...
<div id="leftbar" class="span2">
	...
	<div class="submenu">
	<%
	    Menu rootMenu = (Menu)SecurityUtils.getSubject().getSession().getAttribute("menus");
	    if(rootMenu!=null){
			List<Menu> list = (List<Menu>)rootMenu.getChildrens();
			for(Menu menu : list){
				%>
				  <a id="web-tab" href="${ctx}/<%=menu.getMenuUrl() %>"><%=menu.getMenuName() %><br></a>
					<ul>
					<% 
						List<Menu> childList = (List<Menu>)menu.getChildrens();
						for(Menu childMenu : childList){
					%>
							<li><a id="web-tab" href="${ctx}/<%=childMenu.getMenuUrl() %>"><%=childMenu.getMenuName() %><br></a></li>
					<%
						}
					%>
					</ul>

...
/**
 * 类简介: 使用正则表达式验证数据或提取数据,类中的方法全为静态的<br/> 在这里，我将这个类定义成了一个单例， （本来只有static
 * 方法是不new去对象的，也就用不到单例了）<br/> 是因为有一个 regexpHash(HashMap实例)，为了可以动态
 * 添加正规表达式（不知道是不是想的太多了:) ）。
 * 
 * <pre>
...
public final class Regexp {
	 ...
	 * 
	 */
	public static final String date_regexp = "^[0-9]{4}-(((0[13578]|(10|12))-(0[1-9]|[1-2][0-9]|3[0-1]))|(02-(0[1-9]|[1-2][0-9]))|((0[469]|11)-(0[1-9]|[1-2][0-9]|30)))($|\\s([0-1]\\d|[2][0-3])\\:[0-5]\\d\\:[0-5]\\d)";// 匹配日期
	...
	public static Set getBetweenSeparatorStr(final String originStr,
			final String leftSeparator, final String rightSeparator) {
	...
	public static String getBetweenSeparatorParam(final String originStr,final String leftSeparator,final String rightSeparator)
	{
		String param = "";
		if(!StringUtils.isEmpty(originStr))
		{
			param = originStr.substring(originStr.indexOf(leftSeparator)+1,originStr.indexOf(rightSeparator));
		}
		return param;
	}

	public static void main(String a[]) {
		System.out.println(Regexp.isSoftRegexpValidate("192.131.12.12/24",Regexp.ip_regexp));

		/*
		 * PatternCompiler compiler = new Perl5Compiler(); PatternMatcher
		 * matcher = new Perl5Matcher(); MatchResult
		 * matchResult=matcher.getMatch(); String email =
		 * "wuzhi2000@hotmail.com.cn"; String email_regexp =
		 * "^[\\w-]+(\\.[\\w-]+)*@[\\w-]+(\\.[\\w-]+)+$"; String url =
		 * "http://www"; String url_regexp =
		 * "^(?:http|https|ftp)://(\\w+(-\\w+)*)(\\.(\\w+(-\\w+)*))*(\\?\\S*)?$";//
		 * 匹配url String date = "1111-1-9"; String date_regexp =
		 * "^[1-9]{1}[0-9]{0,3}[-\\s][0-9]{1,2}[-\\s][0-9]{1,2}$";// 匹配日期 String
		 * phone = "010-1234567"; String phone_regexp =
		 * "^(?:0[0-9]{2,4}[-\\s]{1}|\\(0[0-9]{2,4}\\))[0-9]{6,8}$|^[0-9]{6,8}$";//
		 * 匹配电话 String icon="/he//fff/aaaq34.gif"; String icon_regexp =
		 * "^(/{0,1}\\w){1,}\\.(gif|dmp|png)$|^\\w{1,}\\.(gif|dmp|png)$"; String
		 * name="\"^"; String number="023"; String pic
		 * ="forum/head_icons/anoymous20050428125334.jpg";
		...
		expression = "#123$#1223$#12w3$#1w23$";
		System.out.println(getBetweenSeparatorStr(expression, "#","$"));
		//getBetweenSeparatorStr(expression, '#', '$')
		
		for(Object s:getBetweenSeparatorStr(expression,  "#","$")){
			System.out.println(s);
		}
		
		String ss = " 1='' AND 1=2 AND 1='' AND 1=4";
		String[] s1 =  ss.split("AND");
		
		for(String s : s1){
			System.out.println(s);
		}
		
		String  s2 = " n ,WHERE  sd , sd ,sd, WHERE1 232323";
	}

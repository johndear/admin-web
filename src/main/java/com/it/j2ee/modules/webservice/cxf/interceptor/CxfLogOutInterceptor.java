...
public class CxfLogOutInterceptor extends AbstractLoggingInterceptor {
	...
	private static final Logger LOG = LogUtils
			.getLogger(CxfLogOutInterceptor.class);
	private static final String LOG_SETUP = CxfLogOutInterceptor.class
			.getName() + ".log-setup";

	public CxfLogOutInterceptor(String phase) {
		super(phase);
	...
	class LoggingCallback implements CachedOutputStreamCallback {
		...
		public LoggingCallback(Logger paramLogger, Message paramMessage,
			...
			this.lim = ((CxfLogOutInterceptor.this.limit == -1) ? 2147483647
					: CxfLogOutInterceptor.this.limit);
		}
		...
		public void onClose(CachedOutputStream cos) {
			LoggingMessage buffer = CxfLogOutInterceptor.this
			...
			try {
				...
				CxfLogOutInterceptor.this.writePayload(buffer.getPayload(),
						cos, encoding, ct);
			} catch (Exception ex) {
			}
			...
			String id = saveMsg(buffer.getPayload().toString(),interfaceName,method );
			message.getExchange().put(LoggingMessage.ID_KEY, id );
			try {
	...
	private class LogWriter extends FilterWriter {
		...
		public void close() throws IOException {
			...
			CxfLogOutInterceptor.this.log(this.logger, buffer.toString());

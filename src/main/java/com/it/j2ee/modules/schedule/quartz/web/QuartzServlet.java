...
public class QuartzServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			/**
			 * 读取web.xml配置中的quartz.properties进行实例化的SchedulerFactory
			 */
			// Initiate a Schedule Factory
			SchedulerFactory schedulerFactory = (SchedulerFactory) this.getServletContext().getAttribute(QuartzInitializerListener.QUARTZ_FACTORY_KEY);
			...
			if("1".equals(flag)){
				...
				JobDetail jobDetail = new JobDetail(jobName,groupName, CPJob.class);
...
//				scheduler.start();
				...
				scheduler.pauseTrigger(triggerName, groupName);//停止触发器  
	            scheduler.unscheduleJob(triggerName, groupName);//移除触发器
			}else{
